...
const RegionSelectionPage = () => {
  ...
  function isGeoFeature(f: any): f is { properties: { ADMIN: string; ISO_A2: string }; geometry: Geometry } {
    return f && f.properties && typeof f.properties.ADMIN === "string" && typeof f.properties.ISO_A2 === "string";
  }

  useEffect(() => {
    if (!geoJson || !mapContainer.current || mapRef.current) return;
    ...
    map.on("load", () => {
      ...
      map.addLayer({
        ...
        paint: {
          "fill-color": [
            "case",
            ["in", ["get", "ADMIN"], ["literal", Object.keys(selectedRegions)]],
            "#f472b6",
            "#22d3ee"
          ],
          "fill-opacity": 0.3,
          "fill-outline-color": "#68fafc",
        },
      }, "country-label");

      map.setPaintProperty("country-label", "text-color", "#111827");
      map.setPaintProperty("country-label", "text-halo-color", "#ffffff");
      map.setPaintProperty("country-label", "text-halo-width", 2);
      map.setPaintProperty("country-label", "text-halo-blur", 0.8);
      map.setLayoutProperty("country-label", "text-font", ["Lato Bold", "Roboto Medium", "Arial Unicode MS Bold"]);
      map.setLayoutProperty("country-label", "text-size", 18);

      map.on("click", "countries-fill", (e) => {
        const feature = e.features?.[0];
        if (!isGeoFeature(feature)) return;

        const { ADMIN: countryName, ISO_A2: iso } = feature.properties;
        const states = State.getStatesOfCountry(iso).map((s) => s.name);

        const popup = new mapboxgl.Popup({ closeButton: false })
          .setLngLat(e.lngLat)
          .setHTML(`
          <div class="rounded-xl bg-white/80 shadow-lg p-4 border border-gray-500/80 min-w-[180px] relative">
            <div class="flex items-center justify-between mb-3">
              <div class="text-gray-900 font-bold text-base">${countryName}</div>
              <button id="popup-close" class="text-gray-600 hover:text-gray-300 text-xl leading-none">&times;</button>
            </div>
            <button id="select-country" class="bg-gradient-to-r from-pink-500 to-fuchsia-500 text-white py-1.5 px-3 rounded-lg w-full mb-2 text-sm font-medium shadow-md hover:brightness-105">Select Country</button>
            <button id="explore" class="bg-gradient-to-r from-sky-500 to-blue-600 text-white py-1.5 px-3 rounded-lg w-full text-sm font-medium shadow-md hover:brightness-105">Explore States</button>
          </div>
        `)
          .addTo(map);
        ...
        setTimeout(() => {
          ...
          document.getElementById("select-country")?.addEventListener("click", () => {
            if (Object.keys(selectedRegions).length < selectionLimit || selectedRegions[countryName]) {
              setSelectedRegions((prev) => ({ ...prev, [countryName]: [] }));
            }
            setShowStates(false);
            setSelectedCountry(null);
            popup.remove();
          ...
          document.getElementById("explore")?.addEventListener("click", () => {
            ...
            if ("coordinates" in feature.geometry) {
              ...
              const coords: any = feature.geometry.coordinates.flat(2);
  ...
  useEffect(() => {
    const fetchGeoJson = async () => {
      ...
      const countryMap: Record<string, string> = Country.getAllCountries().reduce(
        (acc, c) => ({ ...acc, [c.isoCode]: c.name }),
        {}
      );

      data.features = data.features.map((feature: any) => {
        const iso = feature.properties.ISO_A2;
        if (iso && countryMap[iso]) {
          feature.properties.ADMIN = countryMap[iso];
        }
        return feature;
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-[96rem] mx-auto py-4 px-4">
        <div className="flex justify-between items-center mb-4">
          {/* <div className="flex items-center justify-center mb-6 gap-6">
            {["Select Regions", "Frameworks", "Domains", "Models"].map((title, idx) => (
              <div key={idx} className="flex flex-col items-center">
                <div className={`w-6 h-6 flex items-center justify-center rounded-full font-bold text-sm ${idx === 0 ? "bg-cyan-600 text-white" : "bg-gray-200 text-gray-600"}`}>{idx + 1}</div>
                <div className="text-sm mt-1 font-semibold text-gray-700">{title}</div>
              </div>
            ))}
          </div> */}
          {/* <button onClick={() => navigate("/frameworks")} className="text-sm text-cyan-600 hover:underline hover:text-cyan-800">
            Skip
          </button> */}
        </div>

        <div className="flex flex-col lg:flex-row gap-4">
          <div className="lg:w-1/4 w-full bg-white rounded-lg shadow p-4 border border-cyan-400 h-fit">
            <h3 className="text-lg font-semibold mb-2 text-gray-800">Selected Regions</h3>
            {Object.keys(selectedRegions).length === 0 ? (
              <p className="text-gray-500">No regions selected yet.</p>
            ) : (
              <ul className="space-y-2">
                {Object.entries(selectedRegions).flatMap(([country, states]) => {
                  if (states.length === 0) {
                    return [
                    ...
                    return states.map(state => (
                      <li key={`${state}-${country}`} className="flex items-center justify-between">
                        <span className="text-gray-700">{state} <span className="text-xs text-gray-500">({country})</span></span>
                        <button className="text-red-500 hover:text-red-700 text-sm" onClick={() => {
                          setSelectedRegions(prev => {
                            const updated = { ...prev };
                            updated[country] = updated[country].filter(s => s !== state);
            ...
            <div className="mt-6">
              <button
                ...
                disabled={Object.keys(selectedRegions).length === 0}
                className={`w-full py-3 rounded-lg font-medium text-lg font-poppins transition-all duration-300 relative overflow-hidden ${Object.keys(selectedRegions).length > 0
                  ? "bg-gradient-to-r from-cyan-800 to-cyan-600 text-white shadow-lg hover:shadow-xl transform hover:-translate-y-1"
                  : "bg-gray-200 text-gray-400 cursor-not-allowed"
                  }`}
              >
                <span className="relative z-10">Next Page</span>
          ...
          <div className="lg:w-[80%] w-full h-[82vh] rounded-lg overflow-hidden shadow-lg border border-gray-200 relative">
            <div ref={mapContainer} className="w-full h-full rounded-lg" />
            {showStates && selectedCountry && (
              <div style={{ zIndex: 9999 }} className="absolute top-4 right-4 w-64 bg-white rounded-lg shadow-lg p-4 border border-gray-300 z-10">
                <h4 className="text-md font-bold text-gray-800 mb-2">Select States in {selectedCountry}</h4>
                <ul className="max-h-60 overflow-y-auto text-sm">
                  {statesList.map((state) => (
                    <li key={state} className="flex items-center justify-between mb-1">
                      <span>{state}</span>
                      <input
                        type="checkbox"
                        checked={selectedRegions[selectedCountry]?.includes(state) || false}
                        onChange={() => {
                          setSelectedRegions((prev) => {
                            const current = new Set(prev[selectedCountry] || []);
                            current.has(state) ? current.delete(state) : current.add(state);
                            return {
                              ...prev,
                              [selectedCountry]: Array.from(current)
                            };
                          });
                        }}
                ...
                <button
                  ...
                  className="mt-3 w-full py-2 rounded bg-cyan-600 text-white hover:bg-cyan-700"
                >
                  Done Selecting
                </button>
              </div>
...
//   const [selectedRegions, setSelectedRegions] = useState<{ [country: string]: string[] }>({});
//   const [selectedCountry, setSelectedCountry] = useState<string | null>(null);
//   const [statesList, setStatesList] = useState<string[]>([]);
//   const [showStates, setShowStates] = useState(false);
//   const selectionLimit = 5;
...
//               .setLngLat(e.lngLat)
//               .setHTML(
//                 <div class="text-sm font-sans text-gray-800">
//                   <div class="font-bold mb-2">${countryName}</div>
//                   <button id="select-country" class="bg-gradient-to-r from-pink-500 to-cyan-500 text-white px-2 py-1 rounded mb-1 w-full cursor-pointer">Select Entire Country</button>
//                   <button id="explore-country" class="bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-2 py-1 rounded w-full cursor-pointer">Explore States</button>
//                 </div>
//               )
...
//               document.getElementById("select-country")?.addEventListener("click", () => {
//                 if (Object.keys(selectedRegions).length < selectionLimit || selectedRegions[countryName]) {
//                   setSelectedRegions((prev) => ({ ...prev, [countryName]: states }));
//                 }
//                 setShowStates(false);
//                 setSelectedCountry(null);
//                 popup.remove();
...
//               document.getElementById("explore-country")?.addEventListener("click", () => {
//                 setSelectedCountry(countryName);
//                 setStatesList(states);
//                 setShowStates(true);
//                 popup.remove();
...
//   const bounds = new mapboxgl.LngLatBounds();
//   const coords: any = geometry.coordinates.flat(2);
//   coords.forEach(([lng, lat]: number[]) => bounds.extend([lng, lat]));
...
//           mapRef.current!.on("mouseenter", "countries-layer", () => {
...
//       const res = await fetch("https://raw.githubusercontent.com/nvkelso/natural-earth-vector/master/geojson/ne_50m_admin_0_countries.geojson");
//       const data = await res.json();
//       const countryMap : any = Country.getAllCountries().reduce((acc, c) => ({ ...acc, [c.isoCode]: c.name }), {});
...
//         <div className="flex justify-between items-center mb-4">
//           <div className="flex items-center justify-center mb-6 gap-6">
//             {["Select Regions", "Frameworks", "Preferences", "Review"].map((title, idx) => (
//               <div key={idx} className="flex flex-col items-center">
//                 <div className={w-6 h-6 flex items-center justify-center rounded-full font-bold text-sm ${idx === 0 ? "bg-cyan-600 text-white" : "bg-gray-200 text-gray-600"}}>{idx + 1}</div>
//                 <div className="text-sm mt-1 font-semibold text-gray-700">{title}</div>
//               </div>
...
//           <div className="lg:w-1/4 w-full bg-white rounded-lg shadow p-4 border border-cyan-400 h-fit">
//             <h3 className="text-lg font-semibold mb-2 text-gray-800">Selected Regions</h3>
//             {Object.entries(selectedRegions).length === 0 ? (
//               <p className="text-gray-500">No regions selected yet.</p>
//             ) : (
//               Object.entries(selectedRegions).map(([country, states]) => (
...
//             <div ref={mapContainer} className="w-full h-full rounded-lg" />
//             {showStates && selectedCountry && (
//               <div className="absolute top-4 right-4 w-64 bg-white rounded-lg shadow-lg p-4 border border-gray-300 z-10">
//                 <h4 className="text-md font-bold text-gray-800 mb-2">Select States in {selectedCountry}</h4>
//                 <ul className="max-h-60 overflow-y-auto text-sm">
//                   {statesList.map((state) => (
//                     <li key={state} className="flex items-center justify-between mb-1">
...
//                             const current = new Set(prev[selectedCountry] || []);
//                             current.has(state) ? current.delete(state) : current.add(state);
//                             return { ...prev, [selectedCountry]: Array.from(current) };
//                           });
//                         }}
//                         className="accent-cyan-500"
//                       />
